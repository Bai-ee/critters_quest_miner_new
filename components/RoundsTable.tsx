...
export default function RoundsTable() {
  ...
  const [activeTab, setActiveTab] = useState<'rounds' | 'winners' | 'motherlode'>('rounds');
  ...
  const fetchRounds = async () => {
    try {
      const response = await fetch('/api/rounds?limit=1000');
  ...
  const motherlodeRounds = rounds.filter((round) => {
    const hasMotherlodePayout = round.motherlode_tier !== 'None' && (round.ore_motherlode_payout > 0 || round.sol_motherlode_payout > 0);
    return hasMotherlodePayout;
  });
  ...
  return (
    <div className="w-full">
      <div className="mb-6">
        ...
        <div className="flex gap-4 mb-4">
          <button
            ...
            className={`px-6 py-2 rounded-lg transition-colors ${activeTab === 'rounds'
          ...
          <button
            ...
            className={`px-6 py-2 rounded-lg transition-colors ${activeTab === 'winners'
                ? 'bg-yellow-500/20 text-yellow-500 border border-yellow-500/30'
                : 'bg-gray-800/50 text-gray-400 hover:text-gray-300'
              }`}
          >
          ...
          <button
            onClick={() => setActiveTab('motherlode')}
            className={`px-6 py-2 rounded-lg transition-colors ${activeTab === 'motherlode'
                ? 'bg-yellow-500/20 text-yellow-500 border border-yellow-500/30'
                : 'bg-gray-800/50 text-gray-400 hover:text-gray-300'
              }`}
          >
        ...
        <p className="text-gray-400 text-sm">
          {activeTab === 'rounds'
            ? 'Recent mining rounds and rewards.'
            : activeTab === 'winners'
              ? 'Recent round winners and their rewards.'
              : 'All rounds where motherlode hit.'}
        </p>
      </div>
              ...
              {rounds.map((round) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-4 text-white font-mono">#{round.winning_square + 1}</td>
                    <td className="py-4">
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${oreWinner === 'Split'
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left text-gray-400 text-sm border-b border-gray-800">
                ...
                <th className="pb-3 font-medium">Number of Winners</th>
            ...
            <tbody>
              {motherlodeRounds.map((round) => (
                ...
                >
                  ...
                  <td className="py-4">
                    {round.ore_motherlode_payout > 0 ? (
                      <div className="flex items-center gap-1">
                        <span className="text-orange-400">⛏</span>
                        <span className="text-yellow-400">{formatNumber(round.ore_motherlode_payout, 9)}</span>
                      </div>
                    ) : (
                  ...
                  <td className="py-4">
                    {round.sol_motherlode_payout > 0 ? (
                      <div className="flex items-center gap-1">
                        <span className="text-purple-400">≡</span>
                        <span className="text-yellow-400">{formatNumber(round.sol_motherlode_payout, 9)}</span>
                      </div>
                    ) : (
                      <span className="text-gray-500">–</span>
                    )}
            ...
            <tbody>
              {rounds.flatMap((round) =>
                round.winners.map((winner) => (
                  ...
                  >
                    <td className="py-4">
                      <span className={`px-2 py-1 rounded-full text-sm font-bold ${winner.rank === 1
      ...
      {activeTab === 'motherlode' && motherlodeRounds.length === 0 && (
        <div className="text-center py-12 text-gray-400">
          No motherlode rounds data available
        </div>
      )}
    </div>
  );
